               ...
               -> Result<(usize, usize, usize), &'static str>
{
        ...
        if n == 0 {return Err("mat_f: 0 rows");}
        ...
        for i in 0 ..= n {
            if mat_f[i].size() != (n, n) {return Err("mat_f[_]: size mismatch");}
        }
...
impl SDP for PDIPM
{
    ...
    where L: Write
    {
        ...
        let (n, m, p) = check_param(vec_c, mat_f, mat_a, vec_b)?;
        ...
        let tmp = Mat::new(n, n).set_eye();
        let tmp = &mat_f[n] - s_initial * tmp;
        svd.decomp(&tmp); // re-use because of the same size
        for i in 0 .. n {
