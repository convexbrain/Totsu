...
pub trait Cone<F: Float>
{
    ...
    /// * `x` is \\(x\\), a vector to be projected on entry, and shall be replaced with the projected vector on exit.
    ...
    /// 
    /// * `dp_tau` is a diagonal preconditioning vector to be grouped on entry,
