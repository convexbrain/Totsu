...
use crate::linalg::{Slice, SliceBuf, LinAlgEx};
use super::Operator;
        ...
        match self.typ {
            MatType::General(nr, nc) => {
                if colwise {
                    ...
                    for e in y.get_mut() {
                        ...
                        array = rest;
                        *e = L::abssum(&Slice::new_from(col.get()), 1) + *e;
                    }
                }
                else {
                    assert_eq!(nr, y.len());
                    ...
                    let mut array = self.array;
                    for e in y.get_mut() {
                        *e = L::abssum(&Slice::new_from(array.get()), nr) + *e;
            ...
            MatType::SymPack(n) => {
                ...
                for n in 0.. {
                    ...
                    y[n] = L::abssum(&Slice::new_from(col.get()), 1) + y[n];
                    for i in 0.. n {
                        y[i] = y[i] + col[i].abs();
                    }
                    array = rest;
